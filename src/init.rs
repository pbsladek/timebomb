...
# Warn if an annotation expires within this many days
warn_within_days = 14
...
pub fn print_ci_snippet(_stack: &DetectedStack) {
    ...
    println!("    run: timebomb check --warn-within 14d");
    println!();
    println!("{}", divider_colored);
}
...
mod tests {
    ...
    #[test]
    fn test_build_config_toml_contains_tags() {
        let stack = DetectedStack::default();
        let toml = build_config_toml(&stack);
        assert!(
            toml.contains("tags ="),
            "expected 'tags =' in output:\n{}",
            toml
        );
    }

    #[test]
    fn test_build_config_toml_contains_warn_within() {
        let stack = DetectedStack::default();
        let toml = build_config_toml(&stack);
        assert!(
            toml.contains("warn_within_days = 14"),
            "expected 'warn_within_days = 14' in output:\n{}",
            toml
        );
    }
